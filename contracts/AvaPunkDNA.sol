...
contract AvaPunkDNA {
    ...
    string[] private _topType = [
		...
		function deterministicPseudRandomDNA(uint256 _tokenId, address _minter) 
			public
			pure
			returns(uint256)
		{
			...
			bytes32 hashedOarams = keccak256(encodedParams);

			return uint256(hashedOarams);
		}
    ...
    function getAccesoriesTypes(uint8 _dna) 
      public 
      view 
